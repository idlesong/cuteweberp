...
//		$_POST['ChargeFreightCost'] = $_SESSION['Old_FreightCost'];
		...
		//Quincy: save for later use////////////////////
		$_SESSION['CROSS_INV_PO0'] = $_GET['OrderNumber'];
		$_SESSION['CROSS_INV_PO1'] = $_GET['OrderNumber'];
		$_SESSION['CROSS_INV_PO2'] = $_GET['OrderNumber'];
		$_SESSION['CROSS_INV_PO3'] = $_GET['OrderNumber'];
		$_SESSION['CROSS_INV_DEBTORNO'] = $myrow['debtorno'];
		$_SESSION['Items']->LineCounter=0;
		...
		if (db_num_rows($LineItemsResult)>0) {
			...
			while ($myrow=db_fetch_array($LineItemsResult)) {
				...
				$_SESSION['Items']->add_to_cart($myrow['stkcode'],
						...
						$myrow['decimalplaces'],
						$myrow['narrative'],
						$myrow['orderno'], // order no. also need to modify DefineCartClass
						'No',
						-1,  // Quincy modify
						$myrow['taxcatid'],
						$myrow['stkcode'], // replace vTigerProduct with stkcode
						$myrow['itemdue'],
						$myrow['poline']);	/*NB NO Updates to DB */

				$_SESSION['Items']->LineItems[$n]->StandardCost = $myrow['standardcost'];

				/*Calculate the taxes applicable to this line item from the customer branch Tax Group and Item Tax Category */

				$_SESSION['Items']->GetTaxes($n);
				//echo 'StockCode:'.$myrow['stkcode'].'Stk:'.$_SESSION['Items']->LineItems[0]->vtigerProductID;
				$n++;
...
set all the necessary session variables changed by the POST  */
	...
	foreach ($_SESSION['Items']->LineItems as $Itm) {
		...
		//// Quincy: backup data for Invoice cross multiply sales order.
			$_SESSION['CROSS_INV_STKID'] = $_SESSION['Items']->LineItems[$Itm->LineNumber]->vtigerProductID;
			$_SESSION['CROSS_INV_POSTQTY'] = $_POST[$Itm->LineNumber .  '_QtyDispatched'];
			$CROSS_INVOICE_PONO[0] = $CROSS_INVOICE_PONO[1] = $CROSS_INVOICE_PONO[2] =$CROSS_INVOICE_PONO[3] =$_SESSION['ProcessingOrder'];
			//echo 'Customer:'.$_SESSION['CROSS_INV_DEBTORNO'].' stockid:'.$_SESSION['CROSS_INV_STKID'].' Qty'.$_SESSION['CROSS_INV_POSTQTY'].' PO#:'.$CROSS_INVOICE_PONO[0].'<br>';
			
		//Quincy/////////////////////////////////////////////////////////////
			unset($_SESSION['Items']->LineItems);
			$_SESSION['Items']->LineCounter=0;
		//now populate the line items array with the sales order details records //
		// acording to the Qty needed in the post, find multi PO NO.
	
			$LineItemsSQL = "SELECT stockmaster.description,
						stockmaster.controlled,
						stockmaster.serialised,
						stockmaster.volume,
						stockmaster.kgs,
						...
						stockmaster.materialcost +
							stockmaster.labourcost +
							stockmaster.overheadcost AS standardcost
					FROM salesorders, salesorderdetails INNER JOIN stockmaster
					 	ON salesorderdetails.stkcode = stockmaster.stockid
					WHERE salesorderdetails.stkcode = '" .$_SESSION['CROSS_INV_STKID']. "'
					AND salesorderdetails.quantity - salesorderdetails.qtyinvoiced >0
					AND salesorderdetails.orderno = salesorders.orderno
					AND salesorders.debtorno = '" . $_SESSION['CROSS_INV_DEBTORNO'] . "'
			...
			if (db_num_rows($LineItemsResult)>0 ) {
				...
				$TotalOrderQty = 0;
				$n = 0;
				while ($myrow=db_fetch_array($LineItemsResult) AND $TotalOrderQty<$_SESSION['CROSS_INV_POSTQTY']) {
					//echo '_order#'.$myrow['orderno'].'Qty'.$myrow['quantity'].'TaxIn:'.$myrow['taxcatid'];
					$ProductID = $myrow['stkcode'].'-'.$n;
					//echo '<br>ProductID:'.$ProductID;
					$_SESSION['Items']->add_to_cart($ProductID,
							$myrow['quantity'],
							$myrow['description'],
							$myrow['unitprice'],
							$myrow['discountpercent'],
					...
					$LineLeftQty = $myrow['quantity']-$myrow['qtyinvoiced'];
					$TotalOrderQty += $myrow['quantity']-$myrow['qtyinvoiced'];
					$CROSS_INVOICE_PONO[$n] = $myrow['orderno'];
					$n++;
				} // line items from sales order details //
				
				if( $TotalOrderQty - $_SESSION['CROSS_INV_POSTQTY'] > 0){
					$_SESSION['CROSS_INV_FIXEDLINE'] = $n;
					$_SESSION['CROSS_INV_QTYOVER'] = $LineLeftQty - ($TotalOrderQty - $_SESSION['CROSS_INV_POSTQTY']);					
				}else{
					unset( $_SESSION['CROSS_INV_FIXEDLINE'] );
					unset( $_SESSION['CROSS_INV_QTYOVER'] );
				}
				$_SESSION['CROSS_INV_PO0'] = $CROSS_INVOICE_PONO[0];
				$_SESSION['CROSS_INV_PO1'] = $CROSS_INVOICE_PONO[1];	
				$_SESSION['CROSS_INV_PO2'] = $CROSS_INVOICE_PONO[2];
				$_SESSION['CROSS_INV_PO3'] = $CROSS_INVOICE_PONO[3];
				//echo 'POST0123:'.$_SESSION['CROSS_INV_PO0'].'-'.$_SESSION['CROSS_INV_PO1'].'-'.$_SESSION['CROSS_INV_PO2'].'-'.$_SESSION['CROSS_INV_PO3'];				
			} else {
				echo 'there no order,please add one';
			}
	/////////////////////////////////////////////////////////////////////			
		}
...
echo '<P CLASS="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/customer.png" TITLE="' . _('Customer') . '" ALT="">' . ' ' . _('Customer') . ' :<B> ' . $_SESSION['Items']->DebtorNo;
echo '</B>' . _('-') . ' <B> ' . $_SESSION['Items']->CustomerName. '</B>';
echo '[]' . _('Invoice amounts stated in') . ' ' . $_SESSION['Items']->DefaultCurrency . '</CENTER>';
...
foreach ($_SESSION['Items']->LineItems as $LnItm) {
	...
	echo '<TD>'.$LnItm->vtigerProductID.'</TD>
	...
	if($j == $_SESSION['CROSS_INV_FIXEDLINE'] and isset($_SESSION['CROSS_INV_QTYOVER'])){	
		$LnItm->QtyDispatched = $_SESSION['CROSS_INV_QTYOVER'];
	}
	...
	//Quincy modify: check the total???
		...
		foreach ($_SESSION['Items']->LineItems as $OrderLine) {
			$SQL = "SELECT stockmaster.description,
					...
					WHERE stockmaster.stockid='" . $OrderLine->vtigerProductID . "'
			...
			if ($CheckNegRow['mbflag']=='B' OR $CheckNegRow['mbflag']=='M'){
				if ($CheckNegRow['quantity'] < $OrderLine->QtyDispatched){
					prnMsg( _('Invoicing the selected order would result in negative stock. The system parameters are set to prohibit negative stocks from occurring. This invoice cannot be created until the stock on hand is corrected.'),'error',$OrderLine->vtigerProductID . ' ' . $CheckNegRow['description'] . ' - ' . _('Negative Stock Prohibited'));
					$NegativesFound = true;
				}
				...
				$SQL = "SELECT bom.component,
						...
						WHERE bom.parent='" . $OrderLine->vtigerProductID . "'
						AND locstock.loccode='" . $_SESSION['Items']->Location . "'
...
// Quincy modify : replace $_SESSION['CROSS_INV_PO0'] with LineItems,need total the stkcode in one
	$SQL = "SELECT stkcode,
			quantity,
			qtyinvoiced,
			orderlineno
		...
		WHERE completed=0
		AND orderno = '".$_SESSION['CROSS_INV_PO0']."' 
		OR orderno='".$_SESSION['CROSS_INV_PO1']."'
		OR orderno='".$_SESSION['CROSS_INV_PO2']."' 
		OR orderno='".$_SESSION['CROSS_INV_PO3']."'";
	...
	//echo 'ItemQty:'.$_SESSION['Items']->LineItems[0]->Quantity;
	$n=0;
	while ($myrow = DB_fetch_array($Result)) {

		if ($_SESSION['Items']->LineItems[$n]->Quantity != $myrow['quantity'] OR $_SESSION['Items']->LineItems[$n]->QtyInv != $myrow['qtyinvoiced']) {

			echo '<BR>'. _('Orig order for'). ' ' . $myrow['orderlineno'] . ' '. _('has a quantity of'). ' ' .
				$myrow['quantity'] . ' '. _('and an invoiced qty of'). ' ' . $myrow['qtyinvoiced'] . ' '.
				_('the session shows quantity of'). ' ' . $_SESSION['Items']->LineItems[$n]->Quantity .
				' ' . _('and quantity invoice of'). ' ' . $_SESSION['Items']->LineItems[$n]->QtyInv;
...
/*Now insert the DebtorTrans */
	$CrossInvRef = $_SESSION['CROSS_INV_PO0'];
	if($_SESSION['CROSS_INV_PO1'] != $_SESSION['CROSS_INV_PO0']){
		$CrossInvRef = $CrossInvRef.'_'.$_SESSION['CROSS_INV_PO1'];
	}
	if($_SESSION['CROSS_INV_PO2'] != $_SESSION['CROSS_INV_PO0']){
		$CrossInvRef = $CrossInvRef.'_'.$_SESSION['CROSS_INV_PO2'];
	}
	if($_SESSION['CROSS_INV_PO3'] != $_SESSION['CROSS_INV_PO0']){
		$CrossInvRef = $CrossInvRef.'_'.$_SESSION['CROSS_INV_PO3'];
	}	
	...
	foreach ($_SESSION['Items']->LineItems as $OrderLine) {
		...
		if ($_POST['BOPolicy']=='CAN'){
			...
			$SQL = "UPDATE salesorderdetails
				...
				WHERE orderno = " . $OrderLine->$OrderNo . " 
				AND stkcode = '" . $OrderLine->vtigerProductID . "'";
			...
			if (($OrderLine->Quantity - $OrderLine->QtyDispatched)>0){
				...
				$SQL = "INSERT INTO orderdeliverydifferenceslog (
					...
					VALUES (
						" . $OrderLine->SalesOrderNo . ",
						" . $InvoiceNo . ",
						'" . $OrderLine->vtigerProductID . "',
...
/*Now update SalesOrderDetails for the quantity invoiced and the actual dispatch dates. */
		/* Need to get the salesorderdetails.orderlineno accroding to  $OrderLine->SalesOrderNo & $OrderLine->vtigerProductID*/
		$SQL="SELECT salesorderdetails.orderlineno
			FROM salesorderdetails
			WHERE salesorderdetails.stkcode='" . $OrderLine->vtigerProductID . "'
			AND salesorderdetails.orderno = '" . $OrderLine->SalesOrderNo . "'";
		$ErrMsg = _('WARNING') . ': ' . _('Could not retrieve current salesorderdetials');
		$Result = DB_query($SQL, $db, $ErrMsg);

		if (DB_num_rows($Result)==1){
						$LineNoRow = DB_fetch_row($Result);
						$SalesorderdetialsLineno = $LineNoRow[0];
		} else {
			/* There must be some error this should never happen */
			echo' <br>----There must be some errors----<br> ';
			$SalesorderdetialsLineno = 0;
		}
		...
		if ($OrderLine->QtyDispatched !=0 AND $OrderLine->QtyDispatched!="" AND $OrderLine->QtyDispatched) {
			...
			if ($OrderLine->QtyDispatched>=($OrderLine->Quantity - $OrderLine->QtyInv) OR $_POST['BOPolicy']=="CAN"){
				$SQL = "UPDATE salesorderdetails
					...
					completed=1
					WHERE orderno = " . $OrderLine->SalesOrderNo . "
					AND orderlineno = '" . $SalesorderdetialsLineno . "'";
			} else {
				$SQL = "UPDATE salesorderdetails
					SET qtyinvoiced = qtyinvoiced + " . $OrderLine->QtyDispatched . ",
					actualdispatchdate = '" . $DefaultDispatchDate .  "'
			...
			$Result = DB_query("SELECT mbflag FROM stockmaster WHERE stockid = '" . $OrderLine->vtigerProductID . "'",$db,"<BR>Can't retrieve the mbflag");
			...
			if ($MBFlag=="B" OR $MBFlag=="M") {
				...
				will need it later for the stock movement */
					...
					FROM locstock
					WHERE locstock.stockid='" . $OrderLine->vtigerProductID . "'
				...
				$SQL = "SELECT bom.component,
					...
					AND bom.parent='" . $OrderLine->vtigerProductID . "'
					AND bom.effectiveto > '" . Date("Y-m-d") . "'
					AND bom.effectiveafter < '" . Date("Y-m-d") . "'";

				$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('Could not retrieve assembly components from the database for'). ' '. $OrderLine->vtigerProductID . _('because').' ';
				$DbgMsg = _('The SQL that failed was');
				...
				while ($AssParts = DB_fetch_array($AssResult,$db)){
					...
					$SQL = "INSERT INTO stockmoves (
							...
							'" . $AssParts['component'] . "',
							 ...
							 '" . $_SESSION['Items']->Branch . "',
							 " . $PeriodNo . ",
							 '" . _('Assembly') . ': ' . $OrderLine->vtigerProductID . ' ' . _('Order') . ': ' . $_SESSION['ProcessingOrder'] . "',
							 " . -$AssParts['quantity'] * $OrderLine->QtyDispatched . ",
							 " . $AssParts['standard'] . ",
							 0,
							 " . ($QtyOnHandPrior -($AssParts['quantity'] * $OrderLine->QtyDispatched)) . "
						)";
					$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('Stock movement records for the assembly components of'). ' '. $OrderLine->vtigerProductID . ' ' . _('could not be inserted because');
			...
			if ($MBFlag=='B' OR $MBFlag=='M'){
						...
						newqoh,
						narrative )
					VALUES ('" . $OrderLine->vtigerProductID . "',
						10,
						" . $InvoiceNo . ",
						'" . $_SESSION['Items']->Location . "',
						'" . $DefaultDispatchDate . "',
			...
			if ($OrderLine->Controlled ==1){
				...
				The StockSerialMoves as well */
					...
					$SQL = "UPDATE stockserialitems
							...
							WHERE stockid='" . $OrderLine->vtigerProductID . "'
							AND loccode='" . $_SESSION['Items']->Location . "'
...
/*Insert Sales Analysis records */
			...
			$SQL="SELECT COUNT(*),
				...
				AND salesanalysis.stockid " . LIKE . " '" . $OrderLine->vtigerProductID . "'
			...
			if ($myrow[0]>0){  /*Update the existing record that already exists */
				...
				$SQL = "UPDATE salesanalysis
					...
					AND stockid " . LIKE . " '" . $OrderLine->vtigerProductID . "'
...
/*first the cost of sales entry*/
				...
				$SQL = "INSERT INTO gltrans (
					...
					VALUES (
						...
						'" . $DefaultDispatchDate . "',
						" . $PeriodNo . ",
						" . GetCOGSGLAccount($Area, $OrderLine->vtigerProductID, $_SESSION['Items']->DefaultSalesType, $db) . ",
						'" . $_SESSION['Items']->DebtorNo . " - " . $OrderLine->vtigerProductID . " x " . $OrderLine->QtyDispatched . " @ " . $OrderLine->StandardCost . "',
						" . $OrderLine->StandardCost * $OrderLine->QtyDispatched . "
					)";
...
/*now the stock entry*/
				$StockGLCode = GetStockGLCode($OrderLine->vtigerProductID,$db);
	...
	//Post sales transaction to GL credit sales
				$SalesGLAccounts = GetSalesGLAccount($Area, $OrderLine->vtigerProductID, $_SESSION['Items']->DefaultSalesType, $db);
				...
				$SQL = "INSERT INTO gltrans (
					...
					VALUES (
						...
						'" . $_SESSION['Items']->DebtorNo . " - " . $OrderLine->vtigerProductID . " x " . $OrderLine->QtyDispatched . " @ " . $OrderLine->Price . "',
						" . (-$OrderLine->Price * $OrderLine->QtyDispatched/$_SESSION['CurrencyRate']) . "
					)";
				...
				if ($OrderLine->DiscountPercent !=0){
					...
					$SQL = "INSERT INTO gltrans (
						...
						VALUES (
							...
							'" . $_SESSION['Items']->DebtorNo . " - " . $OrderLine->vtigerProductID . " @ " . ($OrderLine->DiscountPercent * 100) . "%',
...
/*Post debtors transaction to GL debit debtors, credit freight re-charged and credit sales */
	...
	echo '<img src="'.$rootpath.'/css/'.$theme.'/images/printer.png" TITLE="' . _('Print') . '" ALT="">' . ' ' . '<A target="_blank" HREF="'.$rootpath.'/PrintCustTrans.php?' . SID . 'FromTransNo='.$InvoiceNo.'&InvOrCredit=Packinglist&PrintPDF=True">'. _('Print this packing list'). ' (' . _('Landscape') . ')</A><BR>';

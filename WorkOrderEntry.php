...
echo '<tr><th class="label">' . _('Packing PO#') . ':</th><td>' . $_POST['WO'] . '</td>';
/*
...
echo '</select></td></tr>';
*/
echo '<th class="label">' . _('Packing Vendor') .':</td> <td><select name="PackingVendor">';
$LocResult = DB_query('SELECT code,description FROM workcentres',$db);
while ($LocRow = DB_fetch_array($LocResult)){
	if ($_POST['PackingVendor']==$LocRow['code']){
		echo '<option selected value="' . $LocRow['code'] .'">' . $LocRow['description'] . '</option>';
...
echo '<tr><th class="label">' . _('Packing PO Date') . ':</th><td>' . $_POST['StartDate'] . '</td>';

if (!isset($_POST['RequiredBy'])){
	$_POST['RequiredBy'] = Date($_SESSION['DefaultDateFormat']);
}

echo '<th class="label">' . _('Comfirm Date') . ':</th>
...
if (isset($WOResult)){
	echo '<tr><th class="label">' . _('Accumulated Costs') . ':</th>
			  <td>' . number_format($myrow['costissued'],2) . '</td></tr>';
}
echo '</table>
		<P><table class="commontable">';
...
echo '<center>';
echo '<hr><input type=submit name="confirm" value="' . _('Update') . '">';

echo '<BR><P><INPUT TYPE=SUBMIT NAME="delete" VALUE="' . _('Delete This Packing PO') . '" onclick="return confirm(\'' . _('Are You Sure?') . '\');">';

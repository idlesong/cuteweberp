...
//Disable the Email function, to make it simple
$MakePDFThenDisplayIt = True;
...
if (isset($MakePDFThenDisplayIt) or isset($MakePDFThenEmailIt)) {
	...
	$line_height=16;
	   ...
	   /*Yes there are line items to start the ball rolling with a page header */
		...
		while ($POLine=DB_fetch_array($result)){
			...
			//Quincy: Show Amounts by default
    ...
    if ($MakePDFThenDisplayIt){
    	...
    	//$pdf->stream();
		$pdf->Output( 'Wafer_PO'.$OrderNo. '.pdf','D');
	...
	if (!isset($_POST['ShowAmounts']) or $_POST['ShowAmounts']!= 'Yes'){
		$_POST['ShowAmounts'] = 'Yes';
	}

...
/************************ BEGIN SQL TRANSACTIONS ************************/
        ...
        if ($IssueItemRow['controlled'] ==1){
            ...
            if ($IssueItemRow['serialised']==1){
                ...
                // Quincy:Add Wafer PO No. that issue to woitems (stockmoves.stkmoveno )  
                $loopPtr=0;     
			...
			if( $loopPtr==0 )
			{
				$NowSerialNo = trim($SerialNo);
			// Quincy:Add Wafer PO No,and trans date. that issue to woitems (stockmoves.pono )							
				$SQL = "SELECT stockmoves.pono,	
					stockmoves.trandate
				FROM stockserialmoves
				INNER JOIN stockmoves ON stockserialmoves.stockmoveno = stockmoves.stkmoveno
				WHERE stockserialmoves.serialno = '$NowSerialNo'
				AND stockmoves.type = 25
				";
				$ErrMsg =  _('Quincy:CRITICAL ERROR A') . '!';
				$Result = DB_query($SQL,$db,$ErrMsg,true);				
				$AlreadyExistsRow = DB_fetch_row($Result);	
				
				//echo 'stockmoves.pono'.':'.$AlreadyExistsRow[0].'<br>';
				//echo 'stockmoves.transdate'.':'.$AlreadyExistsRow[1].'<br>';				
				//echo '_POST.wo:'. $_POST['WO'].'<br>';
				//echo 'woitems.wo:'. $_POST['WO'].'<br>';
	
	                        $SQL = "UPDATE woitems set maincomppo = ".$AlreadyExistsRow[0]."
	                        	WHERE ( wo='" . $_POST['WO'] . "')
	                        ";
	                        $ErrMsg =  _('Quincy:CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The serial stock item record could not be inserted because');
	                        $DbgMsg =  _('Quincy');
	                        $Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, true);	
	                        	                        	                        
	                        $SQL = "UPDATE woitems set maincomprecv = '".$AlreadyExistsRow[1]."'
	                        	WHERE ( wo='" . $_POST['WO'] . "')
	                        ";
	                        $ErrMsg =  _('Quincy:CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The serial stock item record could not be inserted because');
	                        $DbgMsg =  _('Quincy');
...
*/
/* updated qtyissued in workorders£¬WorkOrderIssue.php */
        $UpdateWOResult =DB_query("UPDATE workorders
                        SET costissued=costissued+" . ($QuantityIssued*$IssueItemRow['cost']) . ",
                        		keyqtyissued = keyqtyissued+". $QuantityIssued ."
                        WHERE wo=" . $_POST['WO'],
                    $db,$ErrMsg,$DbgMsg,true);
...
if (!isset($_POST['IssueItem'])){ //no item selected to issue yet
    ...
    if ($IssueItemRow['controlled']==1){ //controlled
        ...
        if ($IssueItemRow['serialised']==1){ //serialised
            ...
            // Quincy: filter the serial items that quantity is 0 //
            ...
            if (DB_num_rows($SerialNoResult)==0){
                ...
                // Quincy: find the SerialNo. ==> find the SerialNo.'s stockmoveno(stockserialmoves)
            }

...
echo '<br>';
echo '<A target="_blank" HREF="'. $rootpath . '/PO_HeaderSearch.php?' . SID . '">'. _('Select Supplier'). '</A><BR>';
echo "<input onclick='javascript:window.open('http://google.com','newwindow', 'toolbar=no');'  type='button' value='New Wafer PO' />";

//echo "<input onclick=\"javascript:window.location.href='$rootpath/PO_Header.php?&NewOrder=Yes'\"  type='button' value='New Wafer PO' />";
echo "<input onclick=\"javascript:window.open('$rootpath/PO_HeaderSearch.php','toolbar=no', 'location=no', 'directories=no', 'status=no', 'menubar=no', 'scrollbars=yes', 'resizable=yes');\"  type='button' value='Supplier' />";
...
if ($_SESSION['RequireSupplierSelection'] == 1 OR !isset($_SESSION['PO']->SupplierID) OR $_SESSION['PO']->SupplierID == '' ) {
	...
	<TR><TD><INPUT TYPE=SUBMIT NAME='SearchSuppliers' VALUE=" . _('Search Now') . ">
	<INPUT TYPE=SUBMIT ACTION=RESET VALUE='" . _('Reset') . "'></TD></TR>
	</TABLE>
	<CENTER>
	</CENTER>";

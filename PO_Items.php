	   ...
	   if (strlen($_POST['ItemDescription'])<=3){
	    ...
	    if ($AllowUpdate == true){
		   ...
		   $_SESSION['PO']->add_to_order ($_SESSION['PO']->LinesOnOrder+1,
						...
						0,
						$GLAccountName);
		   include ('includes/PO_UnsetFormVbls.php'); //Unset all form vars.
	    }
...
if (isset($_SESSION['msg_SelectedItem'])){ /* NewItem is set from the part selection list as the part code selected */
/* take the form entries and enter the data from the form into the PurchOrder class variable */
	$AlreadyOnThisOrder = 0;
	$selectedItem = $_SESSION['msg_SelectedItem'];
...
if (count($_SESSION['PO']->LineItems)>=0){
   ...
   foreach ($_SESSION['PO']->LineItems as $POLine) {
		...
		if ($POLine->Deleted==false) {
			...
			if (isset($_GET['Edit']) and ($_GET['Edit']-1) == $j++){			
			    //echo 'Edit:'.$_GET['Edit'].'J:'.$j;
				...
				echo '<tr>
				<td>' . $_SESSION['PO']->LineItems[$_GET['Edit']]->StockID . '</td>
				<td>' . $_SESSION['PO']->LineItems[$_GET['Edit']]->ItemDescription . '</td>
				<td><input type="text" name="Qty" value=' . $_SESSION['PO']->LineItems[$_GET['Edit']]->Quantity . '></td>
				<td>' . $_SESSION['PO']->LineItems[$_GET['Edit']]->Units . '</td>
				<td><input type="text" name="ReqDelDate" value=' . $_SESSION['PO']->LineItems[$_GET['Edit']]->ReqDelDate . '></td>
				<td><input type="text" name="Price" value=' . $_SESSION['PO']->LineItems[$_GET['Edit']]->Price . '> <input type=submit name="LookupPrice" value="' . _('Lookup Price') . '"></td>	
				<td></td>
				<td><input type=submit name="UpdateLine" value="' . _('Save') . '"> <input type=submit name="Del" VALUE="' . _('Delete') . '"></td>
				</tr>';			
			}else{
	...
	/*Add a new line form for putting in a new line item with default value*/
		...
		function openSubpage(){
		    window.open('$rootpath/h5ItemsSearch.php','toolbar=no', 'location=no', 'directories=no', 'menubar=no', 'scrollbars=yes', 'resizable=yes');
			
		}
		</script>";
		 
		//Add form in a new line		
		echo '<tr><td><input onclick="openSubpage()", , type="button" value="Select products" /></td>';
		//echo \"<td><input onclick=\"javascript:window.open('$rootpath/PO_ItemsSearch.php','toolbar=no', 'location=no', 'directories=no', 'status=no', 'menubar=no', 'scrollbars=yes', 'resizable=yes');\"  type='button' value='Select products' /></td>";
		//<td><input type="text" name="StockID" size=21 maxlength=20 value="' . strtoupper($_POST['StockID']) . '"><input type=submit name="Search" VALUE="' . _('S') . '"></td>
		echo '<td></td>
		<td><input type="text" size=7 maxlength=6 name="Qty" value=' . $_POST['Qty'] . '></td>
		<td></td>
		<td><input type="text" name="ReqDelDate" value=' . $_POST['ReqDelDate'] . '></td>
		<td><input type="text" name="Price" value=' . $_POST['Price'] . '><input type=submit name="Lookup" Value="' . _('Lookup') . '"></td>	
		<td><input type="text" name="ShiptRef" value=' . $_POST['ShiptRef'] . ' > <a target="_blank" href="'.$rootpath.'/ShiptsList.php?' . SID . '&SupplierID=' . $_SESSION['PO']->SupplierID . '&SupplierName=' . $_SESSION['PO']->SupplierName . '">' . _('Show Open') . '</a></td>
		<td><input type=submit name="EnterLine" value="' . _('Save') . '"></td>
		</tr>';
    ...
    echo '<tr><td><input type=submit name="AddLine" value="' . _('Add Line') . '"></td> <td colspan=6 align=right>' . _('TOTAL Excl Tax') . '</td><td align=right><b>' . $DisplayTotal . '</b></td></tr></table>';

if(0){	
	if (!isset($_POST['StockID']) and !isset($_GET['Edit']) and !isset($_GET['AddLine'])) {
...
/*
		...
		<tr><th>' . _('Stock Code') . '</th>
			...
			<th>' . _('Shipment Ref') . '</th>
			</tr>';

		echo '<tr>
			<td><input type="text" name="StockID" size=21 maxlength=20 value="' . strtoupper($_POST['StockID']) . '"><input type=submit name="Search" VALUE="' . _('S') . '"></td>
			<td><input type="text" size=7 maxlength=6 name="Qty" value=' . $_POST['Qty'] . '></td>
			<td><input type="text" size=15 maxlength=14 name="Price" value=' . $_POST['Price'] . '><input type=submit name="Lookup" Value="' . _('Lookup') . '"></td>
			<td><input type="text" size=12 maxlength=11 name="ReqDelDate" value=' . $_POST['ReqDelDate'] . '></td>
			<td><input type="text" size=10 maxlength=9 name="ShiptRef" value=' . $_POST['ShiptRef'] . ' > <a target="_blank" href="'.$rootpath.'/ShiptsList.php?' . SID . '&SupplierID=' . $_SESSION['PO']->SupplierID . '&SupplierName=' . $_SESSION['PO']->SupplierName . '">' . _('Show Open') . '</a></td>
			<td><input type=submit name="EnterLine" value="' . _('Save') . '"></td>
			</tr>';
...
/*
	...
	echo '<th>' . _('Order Quantity:') . '</th>
		<th>' . _('Price') . ':</th>
		<th>' . _('Required Delivery Date') . ':</th>
		<th>' . _('Shipment Ref') . ': </th>
		</tr>';
	echo '<tr>	
		<td>' . $_SESSION['PO']->LineItems[$_GET['Edit']]->ItemDescription . '</td>
		<td><input type="text" size=7 maxlength=6 name="Qty" value=' . $_SESSION['PO']->LineItems[$_GET['Edit']]->Quantity . '></td>
		<td><input type="text" size=15 maxlength=14 name="Price" value=' . $_SESSION['PO']->LineItems[$_GET['Edit']]->Price . '> <input type=submit name="LookupPrice" value="' . _('Lookup Price') . '"></td>
		<td><input type="text" size=12 maxlength=11 name="ReqDelDate" value=' . $_SESSION['PO']->LineItems[$_GET['Edit']]->ReqDelDate . '></td>
		<td><input type="text" size=10 maxlength=9 name="ShiptRef" value=' . $_SESSION['PO']->LineItems[$_GET['Edit']]->ShiptRef . '><a target="_blank" href="'.$rootpath.'/ShiptsList.php?' . SID . 'SupplierID=' . $_SESSION['PO']->SupplierID . '&SupplierName=' . $_SESSION['PO']->SupplierName . '">' . _('Show Open') . '</a></td>
		<td><input type=submit name="UpdateLine" value="' . _('Save') . '"> <input type=submit name="Del" VALUE="' . _('Delete') . '"></td>
		</tr>';
...
/* 
	...
	echo '<input type="hidden" name="LineNo" value=' . ($_SESSION['PO']->LinesOnOrder + 1) .'>';
		...
		echo '<tr>
			<td><input type="text" name="StockID" size=21 maxlength=20 value="' . strtoupper($_POST['StockID']) . '"> <input type=submit name="Search" VALUE="' . _('S') . '"></td>
			<td><input type="text" size=7 maxlength=6 name="Qty" value=' . $_POST['Qty'] . '></td>
			<td><input type="text" size=15 maxlength=14 name="Price" value=' . $_POST['Price'] . '><input type=submit name="LookupPrice" Value="' . _('Lookup') . '"></td>
			<td><input type="text" size=12 maxlength=11 name="ReqDelDate" value=' . $_POST['ReqDelDate'] . '></td>
			<td><input type="text" size=10 maxlength=9 name="ShiptRef" value=' . $_POST['ShiptRef'] . ' > <a target="_blank" href="'.$rootpath.'/ShiptsList.php?' . SID . '&SupplierID=' . $_SESSION['PO']->SupplierID . '&SupplierName=' . $_SESSION['PO']->SupplierName . '">' . _('Show Open Shipments') . '</a></td>
			</tr>';

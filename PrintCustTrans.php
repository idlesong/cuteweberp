...
if (isset($_GET['EdiSent'])){
	$EditSent = $_GET['EdiSent'];
	//if($EdiSent){
		$sql = "UPDATE debtortrans SET edisent =edisent+1
		WHERE debtortrans.transno='" . $FromTransNo . "'
		AND debtortrans.edisent<2";  
		$msg = _('The Edi sent flag is set!');		
...
//	define('FPDF_FONTPATH','font/');
	...
	} else if($InvOrCredit=='Packinglist'){//Quincy
		$pdf->addinfo('Title',_('Packing List') );
		$pdf->addinfo('Subject',_('Packing List') . ' ' . $FromTransNo . ' ' . _('to') . ' ' . $_POST['ToTransNo']);
	} else {
	   ...
	   if (DB_num_rows($result)==1){
		...
		if ($InvOrCredit=='Invoice'){
			 ...
			 $sql = 'SELECT stockmoves.stockid,
					...
					salesorders.customerref,
					stockmaster.units
			 FROM stockmoves, stockmaster,salesorders
				WHERE stockmoves.stockid = stockmaster.stockid
				AND salesorders.orderno = stockmoves.reference
		...
		if (DB_num_rows($result)>0){
		    ...
		    while ($myrow2=DB_fetch_array($result)){
				...
				//Display multiply customer ref				
				$pdf->addText($Left_Margin+330+$i*60, 420, $FontSize, $myrow2['customerref'].'/');
				...
				{
					// Packing Info Function: G.W./N.W./VOL/POP/==>G.W./N.W./VOL/Cartons(1Carton=POP*12*6)	
					$PackageFactor = explode("*", $myrow2['barcode']);
					
					$PlatesPerCarton = $PackageFactor[1]*$PackageFactor[2];
					$TotalCartons += (int)($TotalPlates / $PlatesPerCarton);
					$DisplayCartons = (int)($TotalCartons);

					
					// Process Item Loop
					if( $PackageFactor[0] == 0 ){
						$PackageFactor[0] = $myrow2['quantity'];
					}
					$ItemPlates = (int)($myrow2['quantity'] / $PackageFactor[0]);
					if($myrow2['quantity'] % $PackageFactor[0] != 0) {
						$ItemPlates++;
					}
					
					$EmptyPlatesLeft = $PlatesPerCarton - $TotalPlates % $PlatesPerCarton;
					$TotalPlates += $ItemPlates;
					
					$ItemQtyInLastCarton = $myrow2['quantity'] % ($PackageFactor[0]*$PlatesPerCarton);					
					
					$ItemCartons = (int)($ItemPlates / $PlatesPerCarton);
					if($ItemPlates % $PlatesPerCarton >  $EmptyPlatesLeft) {
						$ItemCartons++;	
						$ItemQtyInLastCarton -= $EmptyPlatesLeft * $PackageFactor[0];
					}	
					
					// Quincy:replace stockid with customer stockid
					$sql3 = 'SELECT custstkid FROM prices
					...
					if( $myrow3[0] != '' ){
						$DisplayStockName = $myrow3[0];
						//echo 'stkID'.$DisplayStockName.'-'.$myrow2['stockid'].'-'.$myrow['debtorno'];
					} else {
						$DisplayStockName = $myrow2['stockid'];
					}
					
					//echo '<br>TotalPlates:'.$TotalPlates.'ItemCartons:'.$ItemCartons.'TotalCartons:'.$TotalCartons;
					$k=0;
					while( $ItemCartons-- >= 0 ){ 	
						$DisplayLines++;
						
						if( $k++ == 0 ){						
							$DisplayQty = $EmptyPlatesLeft * $PackageFactor[0];
						}		
						if($ItemCartons +1== 0){ // Qty less than blank Carton
							$DisplayQty = $ItemQtyInLastCarton;							
						}						
						//echo '<br>Qty:'.$$myrow2['quantity'].'$k:'.$k.'ItemCartons'.$ItemCartons.'DisplayQty:'.$DisplayQty;
						
						$DisplayCartons++;
						if($DisplayCartons == $TotalCartons+1) {
							$ItemSerialNo++;
						} else {
							$ItemSerialNo=1;
						}
						...
						$LineGrossWeight = $DisplayQty * ($myrow2['kgs']); 
						$LineNetWeight = $DisplayQty * ($myrow2['volume']);
						$LineVolume = 0.038;		

						// SubTotals:Qty,Cartons,GW,NW, VOLUME	
						$SubTotalsQty += $DisplayQty;
						$SubTotalsGW += $LineGrossWeight;
						$SubTotalsNW += $LineNetWeight;
						$SubTotalsVolume += $LineVolume;						
										
						$LeftOvers = $pdf->addTextWrap($Left_Margin+3,$YPos,95,$FontSize,$DisplayStockName);
						$LeftOvers = $pdf->addTextWrap($Left_Margin+100,$YPos,245,$FontSize,$myrow2['description']);
						$LeftOvers = $pdf->addTextWrap($Left_Margin+353,$YPos,96,$FontSize,$DisplayQty,'right');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+443,$YPos,35,$FontSize,$LineCarton,'right');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+560,$YPos,96,$FontSize,'-'.$DisplayCartons,'centre');
						//$LeftOvers = $pdf->addTextWrap($Left_Margin+553,$YPos,35,$FontSize,$ItemSerialNo,'centre');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+590,$YPos,50,$FontSize,$LineGrossWeight,'right');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+662,$YPos,120,$FontSize,$LineNetWeight,'centre');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+642,$YPos,120,$FontSize,$LineVolume,'right');
						$YPos -= ($line_height);
					}
				} else {
					...
					$LeftOvers = $pdf->addTextWrap($Left_Margin+443,$YPos,96,$FontSize,$DisplayQty,'right');
			...
			$YPos += ($line_height * $DisplayLines);
			while( $DisplayLines-->0 ){					
				$pdf->addTextWrap($Left_Margin+553,$YPos,35,$FontSize,$DisplayCartons,'centre');
				$YPos -= ($line_height);
			}			
		...
		/*Print a column vertical line */
		$pdf->line($Left_Margin+540, $TopOfColHeadings+12,$Left_Margin+540,$Bottom_Margin+(4*$line_height));

		/*Print a column vertical line */
		$pdf->line($Left_Margin+595, $TopOfColHeadings+12,$Left_Margin+595,$Bottom_Margin+(4*$line_height));
		
		$pdf->line($Left_Margin+640, $TopOfColHeadings+12,$Left_Margin+640,$Bottom_Margin+(4*$line_height));
		$pdf->line($Left_Margin+690, $TopOfColHeadings+12,$Left_Margin+690,$Bottom_Margin+(4*$line_height));		
		...
		if ($InvOrCredit=='Packinglist'){
			...
			$pdf->line($Left_Margin+242, $YPos+$line_height,$Left_Margin+242,$Bottom_Margin);
			
			/*rule off for total */
			$pdf->line($Left_Margin+242, $YPos+(2*$line_height),$Page_Width-$Right_Margin,$YPos+(2*$line_height));
			
			$pdf->addText($Left_Margin+242, $YPos+5+$line_height,$FontSize, _('Sub Total'));
			$LeftOvers = $pdf->addTextWrap($Left_Margin+353,$YPos+5+$line_height,96,$FontSize,$SubTotalsQty,'right');
			$LeftOvers = $pdf->addTextWrap($Left_Margin+483,$YPos+5+$line_height,96,$FontSize,$DisplayCartons,'centre');
			$LeftOvers = $pdf->addTextWrap($Left_Margin+590,$YPos+5+$line_height,50,$FontSize,$SubTotalsGW,'right');
			$LeftOvers = $pdf->addTextWrap($Left_Margin+662,$YPos+5+$line_height,120,$FontSize,$SubTotalsNW,'centre');
			$LeftOvers = $pdf->addTextWrap($Left_Margin+642,$YPos+5+$line_height,120,$FontSize,$SubTotalsVolume,'right');			
...
//if ($InvOrCredit=='Invoice'){
		...
		//};
			 ...
			 $sql = 'SELECT stockmoves.stockid, stockmaster.appendfile, stockmoves.trandate
					FROM stockmoves,
					stockmaster
				WHERE stockmoves.stockid = stockmaster.stockid
				AND stockmoves.type=10
...
while ($row=DB_fetch_array($result)){
    if ($row['appendfile'] !='0' AND $row['appendfile'] !=='none') {
	 ...
	 // If the appendfile field is empty and EMAIL is not selected, just print the invoice without any appended pages       
		$pdf->setFiles(array($_SESSION['reports_dir'] . '/Invoice.pdf'));
        $pdf->concat();
        if($InvOrCredit=='Packinglist'){ // Quincy file name: Date_Pakinglist_Customer($myrow['name'])
        	
		//$pdf->Output( $myrow['debtorno'].'_SO'.$FromTransNo. '_Packinglist.pdf','D');
		$pdf->Output( $myrow['debtorno'].'_'.$row['trandate']. '_Packinglist.pdf','D');
	}else{
		$pdf->Output( $myrow['debtorno'].'_SO'.$FromTransNo. '_Invoice.pdf','D');
...
/*	if ($len <1020){
	...
	/*Now the customer charged to details in a sub table within a cell of the main table*/
				...
				if ($InvOrCredit=='Invoice') {
				   ...
				   echo "<TR>
				   		<TD BGCOLOR='#EEEEEE'>" .$myrow['brname'] . '<BR>' . $myrow['braddress1'] . ', ' . $myrow['braddress2'] . ', ' . $myrow['braddress3'] . '<BR>' . $myrow['braddress4'] . '<BR>' . $myrow['braddress5'] . '<BR>' . $myrow['braddress6'] . '</TD>';

				   	echo "<TD BGCOLOR='#EEEEEE'>" . $myrow['deliverto'] . '<BR>' . $myrow['deladd1'] . ', ' . $myrow['deladd2'] . ', ' . $myrow['deladd3'] . '<BR>' . $myrow['deladd4'] . '<BR>' . $myrow['deladd5'] . '<BR>' . $myrow['deladd6'] . '</TD>';
				   echo '</TR>
				   </TABLE><HR>';

				   //Quincy: //
				   $ConsignmentRef = $myrow['consignment'];
				   echo "<TABLE WIDTH=100%>
				   		<TR>
	...
	/*Print out the invoice text entered */
				...
				echo '</TABLE>';
				echo '<br> Print Invoice  Print Packinglist Update Consignment Ref<br>';
				echo '<img src="'.$rootpath.'/css/'.$theme.'/images/printer.png" TITLE="' . _('Print') . '" ALT="">' . ' ' . '<A target="_blank" HREF="'.$rootpath.'/PrintCustTrans.php?' . SID . 'FromTransNo='.$FromTransNo.'&InvOrCredit=Invoice&PrintPDF=True">'. _('Print this invoice'). ' (' . _('Landscape') . ')</A><BR>';
				echo '<img src="'.$rootpath.'/css/'.$theme.'/images/printer.png" TITLE="' . _('Print') . '" ALT="">' . ' ' . '<A target="_blank" HREF="'.$rootpath.'/PrintCustTrans.php?' . SID . 'FromTransNo='.$FromTransNo.'&InvOrCredit=Packinglist&PrintPDF=True">'. _('Print this packing list'). ' (' . _('Landscape') . ')</A><BR>';
			} /* end of check to see that there was an invoice record to print */
			$FromTransNo++;
		} /* end loop to print invoices */
